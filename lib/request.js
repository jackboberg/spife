...
class Impl {
  ...
  get id () {
    ...
    this._id = (
      this.server.opts.isExternal
        ? hashIncoming(
            KNORK_TO_REQ.get(this.kreq).headers,
            this.server.opts.requestIDHeaders || ['request-id']
          )
        : KNORK_TO_REQ.get(this.kreq).headers['request-id'] || null
    )
    return this._id
  }
