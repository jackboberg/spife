...
class Impl {
  constructor (kreq, server) {
    ...
    this._getBody = () => {
      return getBody(
        this,
        KNORK_TO_REQ.get(kreq),
        Boolean(server.opts.enableFormParsing)
      )
    }
...
function getBody (impl, req, enableFormParsing) {
  var resolve = null
  var reject = null
  ...
  function collectBody () {
    ...
    req.pipe(new Writable({
      ...
      return Promise.try(() => {
        if (enableFormParsing &&
            req.headers['content-type'].slice(0, 33) ===
            'application/x-www-form-urlencoded') {
          return querystring.parse(Buffer.concat(acc).toString('utf8'))
        }
