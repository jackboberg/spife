...
function builder (yargs) {
  yargs
    .command('app <name>', 'create a new app within a project', buildApp, handleApp)
    .command('project <name> [target]', 'create a new project', buildProject, handleProject)
    .demandCommand()
}

function buildApp (yargs) {
...
function handleProject (argv) {
    ...
    Dir({
      ...
      lib: Dir({
        ...
        'settings.js': File(
          ...
          '  ],\n' +
          `  NAME: ${JSON.stringify(argv.target)},\n` +
  ...
  child.on('exit', code => {
    const child = spawn('npm', ['install', '@npm/knork'], {
      env: Object.assign({}, process.env),
      cwd: path.resolve(target),
      stdio: 'inherit'
    })
